...
const setColumnsOrderForTable = (table = [], columnsOrder = [], orderHandler) => table.map(row => orderHandler(row, columnsOrder));
...
const addColumnForHeader = (header = [], newColumn = '') => [...header, newColumn];
...
const formatRow = (row = [], header = [], config = {}) => row.map((cell, index) => {
...
const joinTable = (rows = [], header = [], config = {}, rowsSeparator = '\n', cellsSeparator = '') => {
    return rows.map(row => {
...
const main = (data) => {
    ...
    const table = setColumnsOrderForTable(slicedLines, [0, 1, 2, 3, 4], rowOrderHandler);
    const max = getMaxColumnValue(slicedLines, 3);
    const expandedHeader = addColumnForHeader(header, 'densityPercentage');
    const tableWithDensity = addCalculatedColumnForRows(table, (row) => Math.round((row[3] * 100) / max));
